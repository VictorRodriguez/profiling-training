...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "Running GOOD version on: "
        ...
        for (int r = 0; r < repeats; ++r) {
             ...
             * FIX
             *
             * Launch the complete matrix operation as one 2-D GPU kernel.
             *
             * The arithmetic is intentionally kept simple so the training
             * focuses on launch granularity and GPU utilization rather than
             * changing the algorithm at the same time.
             */
            q.parallel_for(
                sycl::range<2>(
                ...
                [=](sycl::id<2> idx) {
                    ...
                    const int col = static_cast<int>(idx[1]);
                    float sum = 0.0f;
